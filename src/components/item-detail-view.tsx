    ...
    <div className="flex flex-col border rounded-lg shadow-sm">
      <div className="p-4 border-b bg-muted/40 flex items-center">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">${groupedItem.price.toFixed(2)} each</p>
        </div>
      </div>
      ...
      <div className="p-4 flex justify-between">
        ...
        <div className="space-x-2">
          ...
          <Button variant="outline" size="sm" onClick={handleDeselectAll} disabled={!someSelected}>
            Deselect All
          </Button>
        </div>
      </div>
      ...
      <div className="divide-y">
        {items.map((item, index) => (
          <div
            key={item.id}
            className={`px-4 py-3 flex items-center justify-between ${item.isSelected ? "bg-primary/5" : ""}`}
          >
